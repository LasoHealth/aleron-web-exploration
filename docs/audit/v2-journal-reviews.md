...
| 68 | ~~"Addenda: None…linked, separately signed entry."~~ now "Versions: 1. No amendments." | `Canvas FHIR:DocumentReference.status` | **OK — resolved 15 Sep 2026** | The addendum model was the wrong one, so its missing link stopped mattering. An amendment is a **re-signature**: Canvas mints a second document and flips the first to `status: superseded`, itself. `status` is the chain; `relatesTo` is not needed. Measured in `INSTANCE-FINDINGS.md`, *amending a signed note supersedes its document and mints a new one*. |
| 69 | `Amend` (button) | `Canvas NoteStateChangeEvent` (SGN → ULK → LKD → SGN) | **OK — resolved 15 Sep 2026** | Canvas's own `stateTransitionMatrix` names the SGN → ULK transition **Amend**. Driven in the UI on this instance; the HTTP route is blocked by a separate defect (blank `noteChecksum`), not by the model. |
...
| 90 | ~~"amends by appending a linked, separately signed addendum"~~ now "amends by unlocking, changing, and signing again" | `Canvas FHIR:DocumentReference.status` | **OK — resolved 15 Sep 2026** | "Linked" was the unimplementable word and the sentence no longer uses it. See 68. |
...
5. ~~**Name the addendum-link gap (68, 69, 90).**~~ **Done, and the answer was to drop the addendum.** Measured 15 Sep 2026: amendment is a re-signature, Canvas marks the earlier document `superseded` on its own, and no link field is required. The screen now says "Versions" and "Amend" rather than "Addenda" and "Append addendum". *Closed.*
