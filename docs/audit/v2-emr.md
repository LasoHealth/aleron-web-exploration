...
5. **Move `ServiceRequest`, `canvas_patient_id` and `docs/product/ROLE_AND_RELEASE_MATRIX_V1.md` off the product surface.** All three are BRIEF `api-on-product-surface` violations, and two of them are in a `fixed` annotation that claims they were already removed. *Small.*
